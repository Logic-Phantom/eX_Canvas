...
(function(){
	cpr.core.Module.define("module/canvas/openApiPlanner", function(exports, globals, module){
		...
		exports.skeleton = function(poModel, poCanvas) {
			if (poModel.source != null && poModel.source.kind == "sample") {
				return sampleSkeleton(poModel, poCanvas);
			}
			var vaOut = [];
			var vaNotes = [];
		...
		/**
		 * 응답 샘플 모델의 뼈대 : 조회 버튼(바인딩 없음) → DataSet 마다 제목 + 그리드(남는 높이를 나눠 위아래로) → 폼 DataMap 이 있으면 제목 + 폼 → 닫기.
		 * 그리드 1개 + 폼(8칸 이하)은 좌우(P3-2), 그 밖은 위아래(P1-1 · P2-1 · P3-1 · P1-6). 페이지/부가 정보 DataMap 은 모델에만 둔다.
		 * 좌표 규칙은 skeleton() 과 같아 templatePlanner.planByRule 이 같은 구조로 읽는다.
		 */
		function sampleSkeleton(poModel, poCanvas) {
			var vaOut = [];
			var vaNotes = [];
			var vnWidth = Math.max(LAYOUT.minWidth, Math.round((poCanvas && poCanvas.width) || 800));
			var vnHeight = Math.max(LAYOUT.minHeight, Math.round((poCanvas && poCanvas.height) || 600));
			...
			var vnY = LAYOUT.margin;
			var fId = makeIdPool();
			var vaSets = poModel.datasets || [];
			var vaForms = (poModel.datamaps || []).filter(function(poMap) {
				return poMap.role == "form" && poMap.columns.length > 0;
			});
			var vaOthers = (poModel.datamaps || []).filter(function(poMap) {
				return !(poMap.role == "form" && poMap.columns.length > 0);
			});
			var voForm = vaForms[0] || null;
			var vsPattern = "P1-1";
			var vnFormBottom = 0;

			function gridTitle(poSet, pnIdx) {
				return vaSets.length == 1 ? "목록" : "목록 (" + (poSet.jsonKey || poSet.id) + ")";
			}

			// ── 조회 줄 : 응답 샘플에는 요청 파라미터도 API 도 없어 조회 버튼만(바인딩 없음).
			if (vaSets.length > 0) {
				rightButtons(vaOut, vnRight, vnY, [{
					text : "조회",
					id : "btnSearch",
					bind : "",
					style : "primary"
				}], fId);
				vnY += LAYOUT.rowHeight + LAYOUT.gap;
				vaNotes.push("응답 샘플에는 조회 조건(요청 파라미터)과 API 주소가 없어 조회 버튼만 두었습니다 — Submission 을 만든 뒤 버튼의 Bind 에 sub:<id> 를 넣으세요.");
			}
			var vnDataHeight = vnFooterTop - LAYOUT.gap - vnY;

			if (vaSets.length == 1 && voForm != null && voForm.columns.length <= 8) {
				// 그리드 | 폼 좌우
				vsPattern = "P3-2";
				var vnHalf = Math.floor((vnFull - LAYOUT.gap) / 2);
				var vnRightX = vnLeft + vnHalf + LAYOUT.gap;
				vaOut.push(item("output", gridTitle(vaSets[0], 0), vnLeft, vnY, 160, LAYOUT.rowHeight));
				vaOut.push(item("grid", gridHeaders(vaSets[0]), vnLeft, vnY + vnTitleRow, vnHalf, vnDataHeight - vnTitleRow, "ds:" + vaSets[0].id, fId("grdList")));
				vaOut.push(item("output", "상세 정보", vnRightX, vnY, 160, LAYOUT.rowHeight));
				vnFormBottom = vnY + vnTitleRow + placeFields(vaOut, voForm.columns, voForm.id, vnRightX, vnY + vnTitleRow, vnHalf, 1, true, fId, "");
			} else {
				// 위아래 : 폼은 필요한 높이만큼(미리 재어 둔다), 그리드들이 남는 높이를 나눠 쓴다.
				var vnFormUsed = voForm != null ? placeFields([], voForm.columns, voForm.id, 0, 0, vnFull, 2, true, null, "") : 0;
				var vnFormBlock = voForm != null ? vnTitleRow + vnFormUsed + LAYOUT.gap : 0;
				var vnGridArea = vnDataHeight - vnFormBlock;
				var vnMinGrid = vnTitleRow + LAYOUT.rowHeight * 4;
				if (vaSets.length > 0 && vnGridArea < vnMinGrid * vaSets.length + (vaSets.length - 1) * LAYOUT.gap) {
					vnGridArea = vnMinGrid * vaSets.length + (vaSets.length - 1) * LAYOUT.gap;
					vaNotes.push("그리드·폼이 캔버스 높이를 넘어 아래로 이어 놓았습니다(캔버스 스크롤).");
				}
				var vnEach = vaSets.length > 0 ? Math.floor((vnGridArea - (vaSets.length - 1) * LAYOUT.gap) / vaSets.length) : 0;
				vaSets.forEach(function(poSet, pnIdx) {
					vaOut.push(item("output", gridTitle(poSet, pnIdx), vnLeft, vnY, 160, LAYOUT.rowHeight));
					vaOut.push(item("grid", gridHeaders(poSet), vnLeft, vnY + vnTitleRow, vnFull, vnEach - vnTitleRow, "ds:" + poSet.id, fId(pnIdx == 0 ? "grdList" : "grd" + pascalWords(poSet.id.replace(/^ds/, "")))));
					vnY += vnEach + LAYOUT.gap;
				});
				if (voForm != null) {
					vaOut.push(item("output", "상세 정보", vnLeft, vnY, 160, LAYOUT.rowHeight));
					vnFormBottom = vnY + vnTitleRow + placeFields(vaOut, voForm.columns, voForm.id, vnLeft, vnY + vnTitleRow, vnFull, 2, true, fId, "");
				}
				if (vaSets.length == 0) {
					vsPattern = "P1-6";
				} else if (voForm != null) {
					...
					vsPattern = vaSets.length >= 2 ? "P2-1" : "P1-1";
				}
			}

			vaForms.slice(1).forEach(function(poMap) {
				vaNotes.push("DataMap " + poMap.id + " 은 모델에만 넣었습니다(폼은 하나만 깝니다 — 입력을 놓고 Bind 에 dm:" + poMap.id + ".<컬럼> 을 넣으세요).");
			});
			vaOthers.forEach(function(poMap) {
				vaNotes.push("페이지/부가 정보 DataMap " + poMap.id + " 은 모델에만 넣고 캔버스에는 놓지 않았습니다.");
			});
			if (vaSets.length == 0 && voForm == null) {
				vaNotes.push("그리드로 놓을 DataSet 도 폼으로 놓을 DataMap 도 없어 버튼만 놓았습니다.");
			}
			if (vnFormBottom + LAYOUT.gap > vnFooterTop) {
				vnFooterTop = vnFormBottom + LAYOUT.gap;
				vaNotes.push("상세 폼이 캔버스 높이를 넘어 하단 버튼을 폼 아래(" + vnFooterTop + "px)로 내렸습니다(캔버스 스크롤).");
			}
			rightButtons(vaOut, vnRight, vnFooterTop, [{
				text : "닫기",
				id : "btnClose",
				bind : "",
				style : "secondary"
